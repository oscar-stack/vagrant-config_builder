  ...
  def to_proc
    Proc.new do |vm_config|
      vm_config.provider 'aws' do |config|
        ...
        with_attr(:iam_instance_profile_name) { |val| config.iam_instance_profile_name = val }
        ...
        with_attr(:monitoring)                { |val| config.monitoring                = val }
        with_attr(:ebs_optimized)             { |val| config.ebs_optimized             = val }
        with_attr(:source_dest_check)         { |val| config.source_dest_check         = val }
        ...
        with_attr(:kernel_id)                 { |val| config.kernel_id                 = val }
        with_attr(:tenancy)                   { |val| config.tenancy                   = val }
      end
    end
  end
